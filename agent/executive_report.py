"""
Motor compartilhado para relatorios executivos.
"""
    ...
    if "overview" in include_sections:
        questions.extend(
            [
                ...
                f"Qual foi o workflow que mais executou nos {period}?",
            ]
        )
...
async def generate_sections(runtime, *, prompt: str, time_range: str, include_sections: list[str]) -> list[dict]:
    sections = []
    for question in select_questions(prompt, time_range=time_range, include_sections=include_sections):
        result = await runtime.ask(question, return_details=True, emit_console=False)
        sections.append(
            {
                "title": question,
